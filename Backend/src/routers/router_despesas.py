...
def post_despesas(despesa: DespesaRequest, db: Session = Depends(get_db),  current_user: User = Depends(president_permission)) -> Despesas:
...
def despesas_update(id:int, despesa_request: DespesaRequest, db: Session = Depends(get_db), current_user: User = Depends(president_permission)) -> Despesas:
    if current_user.cargo != "PRESIDENTE":
        raise HTTPException(status_code=403, detail="Usuário não autorizado")
...
def despesas_delete(id:int, db: Session = Depends(get_db), current_user: User = Depends(president_permission)) -> Despesas:
    if current_user.cargo != "PRESIDENTE":
        raise HTTPException(status_code=403, detail="Usuário não autorizado")
